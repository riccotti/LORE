...
def run_experiment(blackbox, X2E, y2E, idx_record2explain, dataset, anchor_explainer, path_data, verbose=False):
    ...
    while True:
        try:
            ...
            # print(cc_outcome_lore, type(cc_outcome_lore))
            ...
            precision_lore = [1 - np.abs(v - cc_outcome_lore) for v in y2E[covered_lore]]
    ...
    while True:
        try:
            ...
            precision_anchor = [1 - np.abs(v - cc_outcome_anchor) for v in y2E[covered_anchor]]
...
def main():
    ...
    experiments_results = open(path_exp + 'lore_vs_anchor_coverage_precision.csv', 'a')
